instructions_page <- function(key, label, style = "text-align: center; margin-left: 10%; margin-right: 10%; margin-bottom: 20px;") {
  ui <- shiny::div(
    shiny::div(psychTestR::i18n(sprintf("%s_%s", label, key), html = TRUE)),
    if (label == "PFT") shiny::tags$img(src = sprintf("www/images/%s/example.png", label), style = "margin-top: 10px; width: 100%; height: auto;"),
    if (label == "PFT") shiny::tags$img(src = sprintf("www/images/%s/example_folding.png", label), style = "margin-top: 10px; margin-bottom: 20px; width: 100%; height: auto;"),
